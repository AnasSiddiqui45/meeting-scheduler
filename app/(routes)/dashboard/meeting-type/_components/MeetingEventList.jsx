    ...
    return (
        <div className='grid grid-cols-1 mt-10 md:grid-cols-2 lg:grid-cols-3 gap-7'>
            {eventList.length>0?eventList?.map((event,index)=>(
                <div className='flex flex-col gap-3 p-5 border border-t-8 rounded-lg shadow-md'
                ...
                >
                    ...
                    <hr></hr>
                    <div className='flex justify-between'>
                    <h2 className='flex items-center gap-2 text-sm cursor-pointer text-primary'
                    onClick={()=>{
                        onCopyClickHandler(event)
                       
                    }}
                    >
                        <Copy className='w-4 h-4'/> Copy Link </h2>
